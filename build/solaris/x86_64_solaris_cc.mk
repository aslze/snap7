...
ifeq ($(shell uname -p), sparc)
	MEMALIGN = -xmemalign=1i
else
	MEMALIGN =
endif
OS                     :=solaris
CXXFLAGS               := -xO2 -Kpic -g $(MEMALIGN)
Platform               :=$(TargetCPU)-$(OS)
ConfigurationName      :=Release
IntermediateDirectory  :=../temp/unix
OutDir                 := $(IntermediateDirectory)
LinkerName             :=CC -m64 -library=stdcxx4 $(MEMALIGN)
SharedObjectLinkerName :=CC -m64 -library=stdcxx4 -G -Kpic $(MEMALIGN)
DebugSwitch            :=-gstab
IncludeSwitch          :=-I
LibrarySwitch          :=-l
OutputSwitch           :=-o 
...
SourceSwitch           :=-c 
OutputFile             :=../bin/solaris/libsnap7.so
...
##
AR       := ar rcus
CXX      := CC -m64 -library=stdcxx4 $(MEMALIGN)
CC       := cc
CFLAGS   :=
...
$(OutputFile): $(IntermediateDirectory)/.d $(Objects) 
	...
	$(SharedObjectLinkerName) $(OutputSwitch)$(OutputFile) `cat $(ObjectsFileList)` $(LibPath) $(Libs) $(LinkOptions)
	$(RM) $(ObjectsFileList)

$(IntermediateDirectory)/.d:
	@test -d ../temp/unix || $(MakeDirCommand) ../temp/unix
